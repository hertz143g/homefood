...
export function Modal({title,children,close,action,wide=false}:{title:string;children:ReactNode;close:()=>void;action?:ReactNode;wide?:boolean}){return <Dialog open onOpenChange={v=>!v&&close()}><DialogContent showCloseButton={false} className={`native-sheet ${wide?'wide':''}`} aria-describedby={undefined}><header className="sheet-bar"><button onClick={close}>Закрыть</button><DialogTitle>{title}</DialogTitle><div>{action}</div></header><div className="sheet-body">{children}</div></DialogContent></Dialog>}
