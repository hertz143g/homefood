...
export function useHome(){
  ...
  const login=useCallback(async()=>{
    ...
    try{const result=await api('session',{initData:telegram.initData});if(request!==sequence.current)return;token.current=result.token;delete result.token;setData(result);setError('')}
  ...
  const refresh=useCallback(async()=>{
    ...
    try{const result=await api('state');if(request===sequence.current){setData(result);setError('')}}catch{if(request===sequence.current)setError('Нет связи с сервером. Повторим подключение автоматически.')}
  },[api]);
  ...
  return {data,error,loading,telegram,login,mutate,api};
}
